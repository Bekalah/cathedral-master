(()=>{
 ...
 function checkMarks(){
  const owned=new Set();
  (window._artifacts||[]).forEach(a=>{ if(a.unlocked){ const g=(window._grimoires||{})[a.primary_text_id]; if(g) owned.add(g.id); } });
  const earned=marks.filter(m=>m.requires.every(req=>owned.has(req))).map(m=>m.id);
  return earned;
 }
